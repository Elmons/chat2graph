...
class Expander:
    """Expand a workflow configuration with new candidate sections."""

    @abstractmethod
    async def expand(
        self, task_tesc: str, current_config: Dict[str, str], round_context: WorkflowLogFormat
    ) -> Tuple[List[OptimizeAction], OptimizeResp]:
        """Expand ``current_config`` and return both actions and resulting config."""


class LLMExpander(Expander):
    """Use LLM calls to propose workflow modifications.

    Core Idea: Level by level expansion.
    The expander suggests high-level optimization actions and modifies operator and expert.
    First, use LLM to proposes optimization actions based on the current workflow configuration.
    Then, use another LLM call to optimize operator definitions guided by the selected actions.
    ...
    def __init__(self) -> None:
        ...
        self.section_desc = { # descriptions for each context section, use to prompt LLM
    ...
    async def _get_optimize_actions(
        ...
        # call LLM to get optimization actions
    ...
    async def _expand_operator(
        ...
        # filter function to validate LLM output.
        # check if the operator definitions are valid.
        def filter(results: List[JsonValue]) -> JsonValue:
        ...
        # call LLM to get optimized operator definitions
    ...
    async def _expand_experts(
        ...
        # filter function to validate LLM output.
        # check if the expert definitions are valid.
        def filter(results: List[JsonValue]) -> JsonValue:
            for optimize_resp in results:
                # syntax check
                resp = OptimizeResp.model_validate(optimize_resp)
                ...
                # reference check
                # ensure all operators are defined
    ...
    async def _generate(
        ...
        """Execute the LLM call with retries and structured filtering."""
    ...
    async def expand(
        ...
        """Optimize the current workflow by optimizing operators and experts.

        pipeline:
        1. Propose high-level optimization actions using LLM based on current configuration.
        2. Optimize operator configurations guided by selected actions using LLM.
        3. Optimize expert configurations guided by selected actions using LLM.
        """
        ...
        all_result: OptimizeResp = OptimizeResp(modifications=[], new_configs={}) # store all optimization results  # noqa: E501
        ...
        if len(operator_optimize_actions) > 0:
            ...
            while retry_count < self.max_retry:
                ...
                # call _expand_operator to optimized operators
        ...
        if len(expert_optimize_actions) > 0:
            ...
            while retry_count < self.max_retry:
                ...
                # call _expand_experts to optimize experts
