...
## Role
You are a workflow optimization analyst for graph-query agents.

## Inputs
- objective_score: {avg_score}
...
## Goal
Produce high-signal reflection for the next optimization round.
Focus on causes that are both:
1) frequent across failed/regressed tasks, or
2) explicitly supported by runtime errors.

## Rules
1. Ignore solved/historical issues not evidenced in current results.
2. Prefer capability-level causes (entity resolution, cypher constraints, output formatting, etc.), not vague wording.
3. Do not repeat generic advice already implied by successful tasks.
4. Suggestions must be actionable in workflow terms (operator instruction/output_schema/workflow topology/validation).
5. Keep output compact: 1-3 failed reasons, 1-3 suggestions.
...
Only these action references are allowed in operator `actions`:
{available_actions}
...
5. In single-expert mode, the expert workflow must end with exactly one tail operator (exactly one node with out-degree 0).
...
### Allowed Actions (Strict Whitelist)
When writing operator `actions`, only use the following references:
{available_actions}
...
2. Optimizations must be based on the context information and centered around the tasks the system needs to complete.
3. Hard constraint (single-expert mode): You must output exactly **one** expert, and its `profile.name` must be `{main_expert_name}`.
4. Hard constraint (workflow topology): The `workflow` must be a DAG and must have exactly **one** tail operator (exactly one operator with out-degree 0).
...
In this project refactor iteration, we run the system in **single-expert mode**: there is exactly one Expert which acts as the entry container of the system. Its `profile.name` is fixed to `{main_expert_name}`. Do not create additional experts.
...
5. Single-tail termination: No matter how many parallel branches exist in the middle, all branches must finally converge so that there is exactly one tail operator.
</Expert Writing Guide>
...
7. Before output, self-check the final workflow topology: DAG + exactly one tail operator.
8. Organize the optimization results and output.
