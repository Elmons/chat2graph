...
2. Optimizations must be based on the context information and centered around the tasks the system needs to complete.
3. Hard constraint (single-expert mode): You must output exactly **one** expert, and its `profile.name` must be `"Main Expert"`.
...
In this project refactor iteration, we run the system in **single-expert mode**: there is exactly one Expert which acts as the entry container of the system. Its `profile.name` is fixed to `"Main Expert"`. Do not create additional experts.
